...
The per-firm `TRA-contracts/<firm>/` corpus with raw filings and intermediate summaries is tracked in the repository so a fresh clone reproduces the database build without re-running the EDGAR acquisition. The corpus is also regenerable from EDGAR via the full workflow below if a from-scratch rebuild is needed.

## Workflow

The pipeline runs in eight steps. Steps 1 and 2 produce a candidate-filing parquet and pull the EX-10.* exhibits referenced by it. Steps 3 through 6 build the per-firm corpus under `TRA-contracts/`; steps 7 and 8 aggregate the corpus into the Parquet outputs and the dashboard. A fresh checkout includes `TRA-contracts/` (~321 firm directories, several hundred MB); the build (steps 7-8) runs directly against it. Steps 1-6 only need to run when refreshing from EDGAR or rebuilding from scratch.
...
Python dependencies are managed with [pixi](https://pixi.sh/). The manifest at `pixi.toml` pins the project's Python (>= 3.11), polars (>= 1.0), pyarrow (>= 15.0), httpx (0.28.1-0.29), pytest, and Jupyter packages; `pixi install` resolves and installs them into a project-local environment under `.pixi/`. The manifest targets both `linux-64` and `win-64`, so the same lockfile works on WSL/Linux and on Windows.
...
The per-firm corpus that feeds the build lives under `TRA-contracts/<firm-slug>_<10-digit-CIK>/`. Each firm directory carries the downloaded filings (`.htm`), their markdown companions (`.md`), per-filing annotation files, a `contract_log.md`, and one `*_summary.qmd` per TRA at the firm. This directory is tracked in the repository (it ships with `git clone`); steps 3 through 6 regenerate it from EDGAR when refreshing or rebuilding.
